...
async function sendEmailRestorePassword(data) {
  try {
    const transport = createTransport({
      host: "smtp.gmail.com",
      port: 465,
    ...
    await transport.sendMail({
      from: `CERAMICA GLORIA <${GOOGLE_EMAIL}>`,
      to: data.to,
      subject: `RESTORE PASSWORD FOR USER ${data.name.toUpperCase()}`,
      html: `<!DOCTYPE html>
      <html lang="en">
      <head>
          <meta charset="UTF-8">
          <meta name="viewport" content="width=device-width, initial-scale=1.0">
          <title>Reestablecimiento de Password</title>
      </head>
      <body style="display: flex; justify-content: center; align-items: center; height: 100vh; background-color: #f8f9fa; margin: 0;">
          <div style="text-align: center; padding: 20px; background-color: #d4edda; border: 1px solid #c3e6cb; border-radius: 5px; color: #155724;">
              <h1>AHORA PUEDES REESTABLECER TU CONTRASEÑA</h1>
              <h2>Ingrese su nueva contraseña desde el siguiente enlace:</h2>
              <a href="http://localhost:8080/users/password" style="font-size: 18px; color: #007bff;">http://localhost:8080/users/password</a>
              <p style="font-size: 18px; margin-top: 10px;">Verification Code: <strong>${data.code}</strong></p>
          </div>
      </body>
      </html>
...
export {sendEmail,sendEmailRestorePassword};
